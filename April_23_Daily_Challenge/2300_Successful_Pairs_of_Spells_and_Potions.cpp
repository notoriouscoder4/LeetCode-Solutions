...
public:
    ...
    {
        sort(potions.begin(), potions.end()); // sorted to apply binary search
        vector<int> pairs;

        for (int spell : spells)
        {
            long potion_needed = (success + spell - 1) / spell; // Finding ceil value of potion_needed
...
public:
    ...
    */
    ...
    vector<int> successfulPairs(vector<int> &spells, vector<int> &potions, long long success)
    {
        ...
        for (int spell : spells)
        {
            pairs.push_back(potions.size() - valid_pos(potions, success, spell));
        }
        return pairs;
    }
};