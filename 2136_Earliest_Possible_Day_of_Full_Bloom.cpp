...
public:
    int earliestFullBloom(vector<int> &plantTime, vector<int> &growTime)
    {
        ...
        vector<pair<int, int>> v(n); // creating pair in which first=growTime and second=plantTime
        for (int i = 0; i < n; ++i)
        {
            v[i] = {growTime[i], plantTime[i]};
        }
        // sorting in reverse order w.r.t growTIme
        sort(v.rbegin(), v.rend());
        int maxx = 0;
        int pt = 0;
        for (int i = 0; i < n; ++i)
        {
            // growTime+plantTime+pt+1
            // here, growTime and plantTime are coming from 'v' and plus '1' is for bloom time
            // here, pt is previous plant time which got wasted when we start planting new flower
            maxx = max(maxx, v[i].first + v[i].second + pt + 1);
            pt += v[i].second;
        }
        return maxx - 1;
    }
